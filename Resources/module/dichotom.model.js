...
Dichotom.prototype.getAll = function(_args) {
	...
	var xhr = Ti.Network.createHTTPClient({
		onload : function() {
			var xml = new Ti.App.XMLTools(xhr.responseText);
...
Dichotom.prototype.cacheImages = function(_dichotom_id) {
	...
	function getImage(_args) {
		var xhr = Ti.Network.createHTTPClient({
			onload : function(_e) {
				_args.file.write(xhr.responseData);
			},
			onerror : function(_e) {
		...
		if (decision) {
			for (var a = 0; a < decision.length; a++) {
				if (decision[a].media && decision[a].media[0] && decision[a].media[0]['url_420px']) {
					...
					var file = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, 'ic', Ti.Utils.md5HexDigest(url) + '@2x.png');
					if (!file.exists())
						images.push(url);
				}
			}
